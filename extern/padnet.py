...
def init_net(feature_size, label_size, batch_size, absolute_max=512):
	...
	with tf.name_scope('network'):
		...
		conv1, coefs1 = conv_layer(1, 50, nodes, children, feature_size)
		...
		carry = tf.layers.dense(carry, 100, activation=tf.nn.relu)
	...
	return nodes, children, hidden, (pooling, coefs1)
...
def conv_layer(num_conv, output_size, nodes, children, feature_size):
	...
	with tf.name_scope('conv_layer'):
		nodes_by_conv = [
			conv_node(conv_ii, nodes, children, feature_size, output_size)
			for conv_ii in range(num_conv)
		]
		nodes_by_conv, coefs = zip(*nodes_by_conv)
		return tf.concat(nodes_by_conv, axis=2), list(coefs)

def conv_node(conv_n, nodes, children, feature_size, output_size):
	"""Perform convolutions over every batch sample."""
	with tf.name_scope('conv_op_%d' % conv_n):
