...
def init_net(feature_size, label_size, batch_size, absolute_max=512):
	"""Initialize an empty network."""

	with tf.name_scope('inputs'):
		nodes = tf.placeholder(tf.float32, shape=(batch_size, absolute_max, feature_size), name='tree')
		children = tf.placeholder(tf.int32, shape=(batch_size, absolute_max, None), name='children')

	with tf.name_scope('network'):
		#                    1024  (og # fts per node)
		conv1, coefs1, input_image = conv_layer(1, 50, nodes, children, feature_size)
		conv1, _, _ = conv_layer(1, 50, conv1, children, 50)

		# pooling = pooling_layer(conv1)
		carry = conv1
		# carry = tf.pad(carry, [[0, 0], [0, absolute_max - tf.shape(carry)[1]], [0, 0]])
		carry = tf.reshape(carry, [batch_size, absolute_max * 50])
		# print('padded', carry.get_shape())
		# max_tree_size = tf.shape(children)[1]
		# print(carry.get_shape())
		carry = tf.layers.dense(carry, 10, activation=tf.nn.relu)
		# carry = tf.layers.dropout(carry, 0.5)
		pooling = carry

		carry = tf.layers.dense(carry, absolute_max * 50, activation=tf.nn.relu)
		carry = tf.reshape(carry, [batch_size, absolute_max, 50])

		carry = deconv(1, 50, 30, carry, coefs1, max_children=tf.shape(children)[2])
		# carry, _ = conv_layer(1, 30, carry, children, feature_size)
		# carry = conv_layer(1, 30, carry, coefs1)
		# exit(0)


		# hidden = hidden_layer(pooling, 100, label_size)
		# hidden = tf.layers.dense(pooling, label_size)
		# unpool = unpool_layer(hidden)
		recovered = carry
	...
	return nodes, children, recovered, input_image, (pooling, coefs1)

def deconv(num_conv, input_dim, output_dim, carry, coefs, max_children):
	with tf.name_scope('deconv'):
		# std = 1.0 / math.sqrt(feature_size)
		w_deconv = tf.Variable(tf.truncated_normal([input_dim, 3, output_dim]), name='deconv_Wt')
		# w_deconv = tf.Variable(tf.truncated_normal([input_dim, output_dim, 3]), name='deconv_Wt')
		# w_t, w_l, w_r = (

		# 	tf.Variable(tf.truncated_normal([input_dim, output_dim]), name='Wl'),
		# 	tf.Variable(tf.truncated_normal([input_dim, output_dim]), name='Wr'),
		# )
		init = tf.truncated_normal([output_dim,])
		#init = tf.zeros([output_size,])
		b_deconv = tf.Variable(init, name='deconv_b')

		# print(carry.get_shape())
		# print(w_deconv.get_shape())
		btree = tf.tensordot(carry, w_deconv, [[2], [0]]) + b_deconv

		# print(btree.get_shape())
		# print(coefs[0].get_shape())

		coef = tf.reshape(coefs[0], [1, 512, -1, 3])
		qtree = tf.matmul(coef, tf.transpose(btree, [0, 1, 3, 2]), transpose_b=True)
		# qtree = tf.tensordot(btree[0], coef, [[1], [2]])
		# print(qtree.get_shape())

		return qtree

		# return conv_step(nodes, children, feature_size, w_t, w_r, w_l, b_conv)
	# carry = [batch_size, max nodes, channels]
	# with tf.name_scope('dequant'):
	# 	batch_size = tf.shape(carry)[0]
	# 	max_tree_size = tf.shape(carry)[1]
	# 	channels = tf.shape(carry)[2]
	...
	# 	x = batch_size * max_tree_size
		...
		# # output is (batch_size, max_tree_size, output_size)
		# return tf.nn.tanh(result + b_conv, name='conv'), coef
...
def conv_layer(num_conv, output_size, nodes, children, feature_size):
	...
	with tf.name_scope('conv_layer'):
		...
		nodes_by_conv, coefs, input_image = zip(*nodes_by_conv)
		return tf.concat(nodes_by_conv, axis=2), list(coefs), list(input_image)
...
def eta_r(children, t_coef):
	...
	with tf.name_scope('coef_r'):
		...
		child_indices = tf.multiply(tf.tile(
			tf.expand_dims(
				tf.expand_dims(
					# starts -1 because [0] is itself (parent)
...
def conv_step(nodes, children, feature_size, w_t, w_r, w_l, b_conv):
	...
	with tf.name_scope('conv_step'):
		...
		with tf.name_scope('combine'):
			...
			# output is (batch_size, max_tree_size, output_size)
			return tf.nn.tanh(result + b_conv, name='conv'), coef, children_vectors
...
def loss_layer(logits_node, input_image, batch_size=1):
	"""Create a loss layer for training."""

	# labels = tf.placeholder(tf.int32, (None, label_size,))
	# labels = tf.placeholder(shape=tf.shape(logits_node))
	# p
	# labels = tf.reshape(input_image, tf.shape(logits_node))
	labels = tf.reshape(input_image, [batch_size, 512, -1, 30])
	logits = tf.reshape(logits_node[:, :, 1:, :], [batch_size, 512, -1, 30])
	...
	with tf.name_scope('loss_layer'):
		...
		loss = tf.reduce_mean(tf.square(tf.subtract(labels, tf.nn.tanh(logits))))
