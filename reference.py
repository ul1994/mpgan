...
def message_pass(g, h, k):
	for v in g.keys():
		...
		for neighbor in neighbors:
			...
			# NOTE: Message function M_t = V(h_w of neighbor:w)
			m_w = V[k](h[w])
			# NOTE: Edge encoder E
			m_e_vw = E(e_vw)
			# NOTE: m_w = message from neighbor:w
			# NOTE: m_e_vw = any qualifier for travelling edge v_w
			reshaped = torch.cat( (h[v], m_w, m_e_vw), 1)
			# NOTE: k = timestep : unique update per timestep
			# NOTE: U = update operator
			# NOTE: h (num_verts,) = storage for h_v for All.v
